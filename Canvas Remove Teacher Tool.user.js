...
// @author       Morgan H. McKie (mmckie@fiu.edu)
// @include     https://devfiu.instructure.com/courses/*/users
...
(function() {
    ...
    var buttonRoles = ["teacher","limited_admin","admin", "root_admin"];
    ...
    function add_button() {
        ...
        if (parent) {
            ...
            if (!el) {
                ...
                el.appendChild(icon);
                var txt = document.createTextNode(' Remove Bulk Teacher Enrollments');
                el.appendChild(txt);
    ...
    function getTeachers(){
        ...
        var url;
        if(!currURL){
            url = "/api/v1/courses/"+courseID+"/enrollments?type[]=TeacherEnrollment&per_page=99&page=";
           // url = "/api/v1/courses/"+courseID+"/enrollments?type[]=StudentEnrollment&per_page=99&page=";
        }else{
            url = "/api/v1/courses/"+courseID+"/enrollments?type[]=TeacherEnrollment&page="+nextURL;
        ...
        allTeachers = results.flat();
    }
    function setTeachers(){
        var toAppend = '';
        $.each(allTeachers,function(i,o){
            var dateStr;
            var dateEnrStr;
            ...
            toAppend += '<tr><td><input type="checkbox" id="'+o.id+'" name="Teachers" value="'+o.id+'"></td><td>'+o.user.sortable_name+'</td><td>'+stuSectionName+'</td><td>'+dateEnrStr+'</td><td>'+dateStr+'</td></tr>';
        });
    ...
    function setEvents() {
        array = $.map($('input[name="teachers"]:checked'), function(c){return c.value; });
    }
        ...
        try {
            window.requestAnimationFrame(() => {
                ...
                $('#events_dialog').dialog({
                    'title' : 'Manage Teacher Enrollments',
                    'autoOpen' : false,
                    'closeOnEscape': false,
